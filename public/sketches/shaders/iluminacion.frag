...
void main() {
  ...
  if (tipo == 0) {
    gl_FragColor = gris ? vec4((vec3(luma(texel.rgb))), 1.0) : texel;
  } else if (tipo == 1) {
    gl_FragColor = gris ? vec4((vec3(hsv(texel.rgb))), 1.0) : texel;
  } else if (tipo == 2) {
    gl_FragColor = gris ? vec4((vec3(hsl(texel.rgb))), 1.0) : texel;
  }
}